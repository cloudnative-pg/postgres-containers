...
fetch_postgres_image_version() {
    local suite="$1";
    local item="$2";
	curl -SsL "https://registry.hub.docker.com/v2/repositories/postgis/postgis/tags/?name=alpine&ordering=last_updated&" | \
	  jq -c ".results[] | select( .name | match(\"^${suite}-[0-9.]+-alpine\"))" | \
	  jq -r ".${item}" | \
	  head -n1
}
