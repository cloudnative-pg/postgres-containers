...
generator() {
	...
	for version in "${debian_versions[@]}"; do
		...
		# Initial tags are "major version", "full version", "full version with release"
		# i.e. "14-bullseye", "14.2-bullseye", "14.2-1-bullseye"
		versionAliases=(
				"${version}-${distro}"
				"${postgresImageVersion}-${releaseVersion}-${distro}"
		...
		entries+=(
			"{\"name\": \"Debian ${version} ${distro}\", \"platforms\": \"$platforms\", \"dir\": \"$os/$versionDir\", \"file\": \"$os/$versionDir/Dockerfile\", \"version\": \"$version\", \"tags\": [\"$(join "\", \"" "${versionAliases[@]}")\"]}"
		)
	done
}
